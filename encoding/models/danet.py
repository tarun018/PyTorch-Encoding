...
from .danet_base import BaseNet
...
class DANet(BaseNet):
    ...
    """
    def __init__(self, nclass, backbone, only_pam=False, aux=False, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(DANet, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        DANetHead_ = DANetHeadOnlyPAM if only_pam else DANetHead
        self.head = DANetHead_(2048, nclass, norm_layer)

    def forward(self, x):
        imsize = x.size()[2:]
        _, _, c3, c4 = self.base_forward(x)
        x = self.head(c4)
        return tuple(upsample(p, imsize, **self._up_kwargs) for p in x)
...
class DANetHeadOnlyPAM(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer):
        super(DANetHeadOnlyPAM, self).__init__()
        inter_channels = in_channels // 4
        self.conv5a = nn.Sequential(nn.Conv2d(in_channels, inter_channels, 3, padding=1, bias=False),
                                    norm_layer(inter_channels),
                                    nn.ReLU())
